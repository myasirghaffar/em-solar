...
function adminToolsBase(env: Env): string {
  const fromEnv =
    process.env.NEXT_PUBLIC_EM_TOOLS_URL?.trim() ||
    process.env.EM_TOOLS_APP_URL?.trim() ||
    '';
  if (fromEnv) return fromEnv.replace(/\/$/, '');
  // Local default: em-tools on :3000
  if ((env.ENVIRONMENT ?? '').toLowerCase() === 'development') {
    return 'http://localhost:3000';
  }
  return frontendBase(env);
}
...
export async function sendContactMessageEmails(
  ...
  const adminUrl = `${adminToolsBase(env)}/contact-messages`;
